package com.example.Krechunyakparser.services;
...
public class Parser {
    ...
    private List<Information> writeDataIntoList(Elements adElems, List<Information> informationList, int num, String data) {
        for (Element product : adElems) {
            ...
            foundInfo.setPrice(price.isEmpty()? "Произошла ошибка - на сайте не указана цена товара!" : price);
    ...
    private void outputInformationToConsole(int countUp) {
        System.out.println("Найдено страниц: " + (countUp == 0 ? 1 : countUp));
        System.out.println("Создание файла путём сбора найденной информации..");
    }
    private void informationParser(double perc) {
        String res = String.format("%.2f", perc);
        System.out.println("Прогресс создания: " + res + "%..");
    }
    ...
    public List<Information> parseEachRequest(String data) throws InterruptedException {
        ...
        if (!elem.isEmpty()) {
            ...
            info.setDesc("Ничего не было найдено. Попробуйте еще раз.");
